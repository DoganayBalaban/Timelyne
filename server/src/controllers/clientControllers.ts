...
export const getAllClients = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const result = await ClientService.getAllClients(req.user.id, validatedQuery.data);
...
export const getClientProjects = catchAsync(async (req: AuthRequest, res: Response) => {
    if (!req.user?.id) {
        throw new AppError("Unauthorized", 401);
    }
    const validatedParams = getClientByIdSchema.safeParse(req.params);
    ...
    const projects = await ClientService.getClientProjects(validatedParams.data.id, req.user.id);
    res.json({ message: "Client projects fetched successfully", data: projects });
});

export const getClientInvoices = catchAsync(async (req: AuthRequest, res: Response) => {
    if (!req.user?.id) {
        throw new AppError("Unauthorized", 401);
    }
    const validatedParams = getClientByIdSchema.safeParse(req.params);
    ...
    const invoices = await ClientService.getClientInvoices(validatedParams.data.id, req.user.id);
    res.json({ message: "Client invoices fetched successfully", data: invoices });
});

export const getClientRevenue = catchAsync(async (req: AuthRequest, res: Response) => {
    if (!req.user?.id) {
        throw new AppError("Unauthorized", 401);
    }
    const validatedParams = getClientByIdSchema.safeParse(req.params);
    ...
    const revenue = await ClientService.getClientRevenue(validatedParams.data.id, req.user.id);
