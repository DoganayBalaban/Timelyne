...
import {
  getTimeReportQuerySchema,
  manualTimeEntrySchema,
  startTimeEntrySchema,
  updateTimeEntrySchema,
} from "../validators/timerSchema";

export const startTimeEntry = catchAsync(
...
export const stopTimeEntry = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
    const { id } = req.params;
...
export const getActiveTimeEntry = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
...
export const createManualTimeEntry = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
    const validatedData = manualTimeEntrySchema.parse(req.body);
    const entry = await TimerService.createManualTimeEntry(
      userId,
      validatedData,
    );
    res.status(201).json({
...
export const getTimeReport = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
...
export const getTimeEntryById = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
    const { id } = req.params;
...
export const updateTimeEntry = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    const entry = await TimerService.updateTimeEntry(
      userId,
      id as string,
      validatedData,
    );
